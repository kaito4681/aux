...
class CustomEncoder(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        for layer in self.layers:
            ...
            ret.append(input)
        input = self.ln(input)
        return input, ret[:-1]  # exclude the output of the last layer
...
class VisionTransformer(nn.Module):
    ...
    def get_intermediate_representations(self, x: torch.Tensor):
        # Reshape and permute the input tensor
        x = self._process_input(x)
        n = x.shape[0]
    ...
    def calc_loss(self, x: torch.Tensor, predict_models: list[nn.Module], target: torch.Tensor, criterion: Callable[[torch.Tensor, torch.Tensor], torch.Tensor]):
        out, feats = self.get_intermediate_representations(x)
        last_loss = criterion(out, target)
        loss = 0
        for i, (feat, model) in enumerate(zip(feats, predict_models)):
            pred = model(feat[:, 0])
            loss += criterion(pred, target)
        tot_loss = loss + last_loss
        return tot_loss
