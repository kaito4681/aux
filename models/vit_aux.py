...
class Encoder(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        ...
        return input, encoder_features[
            :-1
        ]  #  最後の層の出力はすでにinputとして返しているのでencoder_featuresでは除外


class VisionTransformerAux(nn.Module):
    ...
    def __init__(
        ...
        self.aux_heads = nn.ModuleList(
            [
                nn.Sequential(
                    OrderedDict([("head", nn.Linear(hidden_dim, num_classes))])
                )
                for _ in range(num_layers - 1)  # 最後の層以外の各層用
            ]
        )
        # 変更(追加終わり)
        ...
        for aux_head in self.aux_heads:
            if isinstance(aux_head.head, nn.Linear):
                nn.init.zeros_(aux_head.head.weight)
                nn.init.zeros_(aux_head.head.bias)
        # 変更(追加終わり)
    ...
    def forward(self, x: torch.Tensor):
        ...
        # 各中間層からも分類結果を計算
        aux_outputs = []
        for feat, aux_head in zip(encoder_features, self.aux_heads):
            aux_feat = feat[:, 0]  # class tokenを抽出
            aux_output = aux_head(aux_feat)
